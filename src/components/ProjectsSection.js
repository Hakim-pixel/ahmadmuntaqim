...
const projects = [
  ...
  },
  {
    ...
    category: 'Web App',
    year: '2026',
    links: { github: 'https://github.com/Hakim-pixel/webparkir', live: 'https://webparkir.vercel.app' },
  },
  {
    id: 5,
    ...
    >
      ...
      <div className="max-w-5xl mx-auto">
          ...
          {cats.map(cat => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-xs font-medium border transition-all duration-200 cursor-pointer ${
                active === cat
                  ? 'bg-accent/10 border-accent/30 text-accent-dim'
                  : 'bg-transparent border-border text-text-secondary hover:border-border hover:text-text-primary'
              }`}
            >
              {cat}
            </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {filtered.map(p => (
            <div key={p.id} className="card p-5 group flex flex-col gap-3 hover:-translate-y-0.5 transition-transform duration-200">
              <div className="flex items-start justify-between gap-2">
                <h3 className="text-sm font-semibold text-text-primary leading-snug">{p.title}</h3>
